...
class XMLRPC_plus_ICP_SignalPanel(AbortPauseFinishPanel):
    icsig = xmlrpclib.ServerProxy('http://'+HOSTNAME+':8000')
    
    def getICPProcess(self):
        procID = None
        if ('icp.lock' in os.listdir('/tmp')):
            try:
                procString = os.readlink('/tmp/icp.lock')
                procID = int(procString.split(':')[-1])
            except:
                pass
        return procID
    
    def sendICPSignal(self, sig):
        icpProcID = self.getICPProcess()
        if icpProcID is not None:
...
class MyFrame(wx.Frame):
    def __init__(self, *args, **kwds):
        ...
        self.abort_panel = XMLRPC_plus_ICP_SignalPanel(self, -1)
