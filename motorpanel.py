...
DEBUG = False
MOTORNAME_FILE = os.path.join(os.path.dirname(__file__), 'motornames.json')
...
class MotorPanel(wx.Panel):
    def __init__(self, parent, id = -1, motors=None):
        ...
        self.__do_layout()
        self.update_all_motors(self.motors)
    ...
    def move_all(self, event = None):
        motnums = []
        positions = []
        for i, show in enumerate(self.motors_enabled):
            motnum = i+1
            if show:
                motnums.append(motnum)
                positions.append(float(self.get_destination(motnum)))
...
class RapidScanPanel(wx.Panel):
    def __init__(self, parent, id = -1, motors=None):
        ...
        self.motor_pos = wx.StaticText(self, -1, 'Unknown')
    ...
    def update_choice_pos(self, evt=None):
        ...
        visible_motors = [motor_id for motor_id in self.motors if self.motors[motor_id]['visible'] is True]
        chosen_motor_id = visible_motors[selection]
        self.motor_pos.SetLabel(str(self.get_motor_pos(self.motors[chosen_motor_id]['motnum'])))
